...
object MapReduce extends App {
  ...
  countWordsInFile("src/main/resources/lorumipsum.txt", 6);
  ...
  class Master(nrOfWorkers: Int, latch:CountDownLatch) extends Actor {
    ...
    override def postStop() {
      ...
      println("Top 10 Result after %s ms :".format(end))
      var c = 10;
      for((key, value) <- resultMap.toList.sortBy(_._2).reverse; if c != 0) {
